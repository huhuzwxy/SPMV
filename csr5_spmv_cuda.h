...
vT candidate(const vT *d_value_partition,const vT *d_x,cudaTextureObject_t d_x_tex,const iT *d_column_index_partition,const iT candidate_index,const vT alpha){
    vT x = 0;
...
vT segmented_sum(vT tmp_sum,volatile vT *s_sum,const int scansum_offset,const int lane_id){
    if (lane_id)
...
__inline__ __device__
void partition_fast_track(const vT *d_value_partition,const vT *d_x, cudaTextureObject_t d_x_tex,const iT *d_column_index_partition,vT *d_calibrator,
                          volatile vT *s_sum,const int lane_id,const iT par_id,const vT alpha){
    vT sum = 0;
...
__inline__ __device__
void partition_normal_track(const iT *d_column_index_partition,const vT *d_value_partition,const vT *d_x,cudaTextureObject_t d_x_tex,const uiT *d_partition_descriptor,
                            const iT *d_partition_descriptor_offset_pointer,const iT *d_partition_descriptor_offset,
                            vT *d_calibrator,vT *d_y,volatile vT *s_sum,volatile int *s_scan,const iT par_id,const int lane_id,const int bit_y_offset,
    ...
    // add s_sum to position stop
    last_sum += (start <= stop) ? sum : 0;
    // write sums to result array
    if (direct)
...
template<typename iT, typename uiT, typename vT, int c_sigma>
__inline__ __device__
void spmv_partition(const iT *d_column_index_partition,const vT *d_value_partition,const iT *d_row_pointer,const vT *d_x,cudaTextureObject_t d_x_tex,
                    const uiT *d_partition_pointer,const uiT *d_partition_descriptor,const iT *d_partition_descriptor_offset_pointer,const iT *d_partition_descriptor_offset,
                    vT *d_calibrator,vT *d_y,const iT par_id,const int lane_id,const int bunch_id,const int bit_y_offset,const int bit_scansum_offset,const vT alpha){
    ...
    if (row_start == row_stop){
        partition_fast_track<iT, vT, c_sigma>(d_value_partition, d_x, d_x_tex, d_column_index_partition,d_calibrator,&s_sum[bunch_id * ANONYMOUSLIB_CSR5_OMEGA],lane_id, par_id, alpha);
    }
	//normal tile track
    else{
        ...
        partition_normal_track<iT, uiT, vT, c_sigma>
                (d_column_index_partition, d_value_partition, d_x, d_x_tex,d_partition_descriptor, d_partition_descriptor_offset_pointer, d_partition_descriptor_offset,
                 d_calibrator, d_y,&s_sum[bunch_id * ANONYMOUSLIB_CSR5_OMEGA],&s_scan[bunch_id * (ANONYMOUSLIB_CSR5_OMEGA + 1)],par_id, lane_id, bit_y_offset, bit_scansum_offset, row_start, empty_rows, alpha);
    }
}

template<typename iT, typename uiT, typename vT, int c_sigma>
__global__
void spmv_csr5_compute_kernel(const iT *d_column_index,const vT *d_value,const iT *d_row_pointer,const vT *d_x,cudaTextureObject_t d_x_tex,const uiT *d_partition_pointer,const uiT *d_partition_descriptor,
                              const iT *d_partition_descriptor_offset_pointer,const iT *d_partition_descriptor_offset,vT *d_calibrator,vT *d_y,const iT p,const int num_packet,
    ...
    spmv_partition<iT, uiT, vT, c_sigma>
                (&d_column_index[par_id * ANONYMOUSLIB_CSR5_OMEGA * c_sigma],&d_value[par_id * ANONYMOUSLIB_CSR5_OMEGA * c_sigma],d_row_pointer, d_x, d_x_tex, d_partition_pointer,
                 &d_partition_descriptor[par_id * ANONYMOUSLIB_CSR5_OMEGA * num_packet],d_partition_descriptor_offset_pointer, d_partition_descriptor_offset,d_calibrator, d_y, par_id, lane_id, bunch_id, bit_y_offset, bit_scansum_offset, alpha);
}

template<typename iT, typename uiT, typename vT>
__global__ void spmv_csr5_calibrate_kernel(const uiT *d_partition_pointer,const vT *d_calibrator,vT *d_y,const iT p){
...
template<typename iT, typename uiT, typename vT>
__global__ void spmv_csr5_tail_partition_kernel(const iT *d_row_pointer,const iT *d_column_index,const vT *d_value,const vT *d_x,cudaTextureObject_t d_x_tex,vT *d_y,const iT tail_partition_start,const iT p,const int sigma,const vT alpha)
{
    const int local_id = threadIdx.x;
...
int csr5_spmv(const int sigma,const ANONYMOUSLIB_IT p,const ANONYMOUSLIB_IT m,const int bit_y_offset,const int bit_scansum_offset,
              const int num_packet,const ANONYMOUSLIB_IT *row_pointer,const ANONYMOUSLIB_IT *column_index,const ANONYMOUSLIB_VT *value,const ANONYMOUSLIB_UIT *partition_pointer,
              const ANONYMOUSLIB_UIT *partition_descriptor,const ANONYMOUSLIB_IT *partition_descriptor_offset_pointer,const ANONYMOUSLIB_IT *partition_descriptor_offset,ANONYMOUSLIB_VT *calibrator,
              const ANONYMOUSLIB_IT tail_partition_start,const ANONYMOUSLIB_VT alpha,const ANONYMOUSLIB_VT *x,cudaTextureObject_t x_tex,ANONYMOUSLIB_VT *y){
    ...
    spmv_csr5_tail_partition_kernel<ANONYMOUSLIB_IT, ANONYMOUSLIB_UIT, ANONYMOUSLIB_VT><<< num_blocks, num_threads >>>(row_pointer, column_index, value, x, x_tex, y,tail_partition_start, p, sigma, alpha);
